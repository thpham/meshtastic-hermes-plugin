...
def test_process_inbound_skips_channel_by_default():
    ...
    assert res["action"] == "skip"  # dms_only default


def test_process_inbound_replies_to_channel_when_allowed():
    res = gb.process_inbound(_channel("spam", channel=3), MY, _echo, dms_only=False)
    assert res["action"] == "reply"
